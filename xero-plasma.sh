...
main_menu() {
  ...
  case "$CHOICE" in
    1)
      install_packages "linux-headers plasma-meta konsole kate dolphin ark plasma-workspace egl-wayland flatpak-kcm breeze-grub spectacle dolphin-plugins falkon nano power-profiles-daemon"
      ;;
    2)
...
if dialog --stdout --title "Enable Repos & Install Toolkit" --yesno "\nWould you like to add the XeroLinux & Chaotic-AUR repositories and install the Xero-Toolkit?\n\nIt is recommended as it will make things like driver and package configuration easier." 12 50; then
  bash -c "$(curl -fsSL https://tinyurl.com/xtoolkit)"
  echo
  echo "Installing other useful applications..."
  install_packages "meld timeshift elisa mpv gnome-disk-utility btop wayland-protocols wayland-utils lib32-wayland downgrade update-grub preload"
